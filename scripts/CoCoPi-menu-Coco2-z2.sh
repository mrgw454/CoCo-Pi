...
while opt=$(zenity --width=640 --height=480 --title="$title" --window-icon="/home/pi/Pictures/CoCo-Pi.png" --text="$prompt" --list  --column=""  "${options[@]}"); do
    ...
    "${options[5]}" ) $HOME/.mame/coco2-hdbdos.sh;;
    "${options[6]}" ) $HOME/.mame/coco2-hdbdos-6309.sh;;
    "${options[7]}" ) $HOME/.mame/coco2-hdbdos-pyDW-PLATO.sh;;
    "${options[8]}" ) $HOME/.mame/coco2-yados-HD-mpi.sh;;
    "${options[9]}" ) $HOME/.mame/coco2-yados-HD-6309-mpi.sh;;
    "${options[10]}" ) $HOME/scripts/menu-z2.sh & kill $$;;
